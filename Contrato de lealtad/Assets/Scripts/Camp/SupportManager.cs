...
public class SupportManager : MonoBehaviour
{
    ...
    public void BorrarDatosGuardados()
    {
        ...
        afectos.Clear(); // Resetear afectos también
    }
    ...
    public void AñadirAfecto(string personajeA, string personajeB, int puntos)
    {
        ...
        // --- NUEVO: chequeamos si tienen una conversación pendiente de ver ---
        if (TieneConversacionPendiente(personajeA, personajeB))
        {
            Debug.Log($"⏸️ {key.Item1} y {key.Item2} tienen un apoyo pendiente de ver. No ganan afecto.");
            return; // No ganan afecto hasta que vean la conversación
        }
        // ----------------------------------------------------------------------
        ...
        Debug.Log($"❤️ {key.Item1} y {key.Item2} ganan {puntos} puntos de afecto. Total: {afectos[key]}");
        ...
        if (afectos[key] == 20 || afectos[key] == 40 || afectos[key] == 60)
        {
            if (TieneApoyo(personajeA, personajeB))
            {
                ...
                else
                {
                    ...
                    Debug.Log($"🕓 Apoyo pendiente entre {key.Item1} y {key.Item2} guardado (requerido: {afectos[key]} puntos)");
                }
            }
        }
        ...
        foreach (var c in datosApoyo.conversaciones)
        {
                ...
                (c.personajeA == personajeB && c.personajeB == personajeA))
            {
                ...
                if (!c.rangosVistos.Contains(rango))
                {
                    ...
                    // --- NUEVO: intentar desbloquear desvío ---
